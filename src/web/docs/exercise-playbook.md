...
`[intro]` slot for `ExercisePromptCard` (pass the same long title as its `heading`), a body, an optional `[editor]` slot (list/assessment
...
- "Read more" opens `ExerciseGuide`, a `MatDialog`: full-screen below `HANDSET_QUERY`, centred and
  ...
  (numbered), An example (§4's "Guide example" column), Afterwards. It stays a dialog on desktop
  too: the side panel #210 §4.9 proposed for desktop is dropped for good (#230).
- Each example has a `kind` (#230): `fields` (the default; a worksheet's labelled fields, one per
  step) or `card` (a list/assessment item, rendered as a read-only mock of its list row — `title`,
  optional `subtitle` and `done` — with its `fields` beneath). Where the exercise branches, give one
  example per branch (Keep and Rewrite; over-used and balanced; a planned and a shared chapter).
- The habit hub's info icon opens the same dialog as "About this habit" (#219, #230): In short
  (`habits.about.<habit>.inShort`) plus the hub's own `extra` section, the habit's exercises in
  order with the hub row's status. A new exercise appears there through `registerExercise()` alone.
...
| A `guide` block in the exercise's own i18n scope — `inShort`, `howTo` (numbered steps), `examples` (§4's "Guide example" column: one card per step/item type/branch, each with a `kind` of `fields` or `card`, §5), `afterwards` — read with `translateObjectSignal` and gated on real content (`howTo.length`, since the signal starts at an empty object before the scope loads) | `features/paradigms-perception/i18n/en.json`'s `guide` key; `perception-page.ts`'s `guideContent`; `exercise-guide.ts`/`.html` |
| A `label`/`prompt`/`placeholder` triple per free-text field: `mat-label` = a short noun, a `<p class="field-prompt">` above the field = the question, `placeholder` = a worked example, worded the same as the guide's example for that field — never the only label (`ReflectionEditor` takes it as its `placeholder` input) | `perception-page.html`'s `step1.firstView.{label,prompt,placeholder}` and every other field |
