...
**Agent start gate.** Before starting any agent against a worktree (coder, tester, a fresh round), check `ListAgents` — not memory of an earlier report. If the previous agent for that same issue/worktree still shows `running`, that is not "odd," it is the answer: don't spawn a second one on the assumption the first has finished. Wait for it to go idle, or message it and wait for its reply, before starting anything else there. Two coders in one worktree can write over each other's uncommitted changes; a tester started against a coder that's still pushing verifies a stale commit. If a duplicate is started anyway and it detects a live writer (uncommitted diff growing, a file's mtime seconds old) and stops itself before writing, don't restart it — resolve which one continues (usually whichever has real, correct progress already) and stop the other with `TaskStop`.
...
- **Fresh agent for the review-fix step too.** Post `/code-review` findings as a PR comment (not just a message to the coder), then default to starting a fresh agent for the fix — not resuming the original — since the comment already carries every file/line the fix needs. Resuming instead re-bills that coder's entire accumulated context (implementation + review) on every turn, and it only grows from there. Only keep the original coder if it is still `running` when the review lands (interrupting a live writer risks a half-edited worktree) — let that round finish, then hand any next round to a fresh agent.
- **Don't let a round sit idle before its next message.** The prompt cache expires after a gap (about an hour normally, ~5 minutes in usage overage); a message sent after that reload re-bills the whole accumulated context at full price instead of the cheaper cache-read rate. Send follow-ups (review findings, answers to a blocking question) as soon as they're ready rather than batching them for later.
...
- **Design check before coding.** Anything touching platform or browser semantics (IndexedDB versioning, Web Locks, storage eviction, service workers, **router/component-reuse semantics for a stateful in-page editor or wizard**) gets a three-sentence "why this works in the failure case the issue describes" comment on the issue *before* implementation. A wrong premise costs a whole round; the comment costs almost nothing. (Router reuse added after #187: sibling routes for a list/editor pair looked fine but made Angular destroy and recreate the whole page on every open/close, costing a full round plus two escalation rounds before review caught it — the same "design fact, not a coding slip" shape as the #139 IndexedDB incident that first motivated this rule.)
