...
| Short title            | `habits.exercises.<exerciseId>.shortTitle`, ≤ 3 words, set as the registry entry's required `shortTitleKey` (#218); the root route title `titles.<exerciseId>` holds the same wording. Chrome (top app bar, tab title, hub list, the hub's "Continue: …" button) shows only the short title; the long `title` (`prompt.title`) is the page's visually hidden `h1` and the visible `heading` of `ExercisePromptCard` | `titles.paradigms-perception` = "Your paradigm" |
