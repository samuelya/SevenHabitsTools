...
| `backend-coder` | claude-opus-5-5 (owner default since 2026-09-24); escalates to Fable | `src/api/**`, `src/api.Tests/**`, `infra/**`, `.github/workflows/**` | touches `src/web`, merges |
| `frontend-coder` | claude-opus-5-5 (owner default since 2026-09-24); escalates to Fable | `src/web/**` | touches api/infra, merges |
...
Pipeline: owner picks an issue → coder in its own worktree (`feat/<issue>-<slug>`) → PR `Closes #n` → owner runs `/code-review` → fresh coder fixes findings → lead runs `wait-ci.sh` (red: `ci-failures.sh --post`, fresh coder) → `tester` (Sonnet) on green CI checks the acceptance criteria (bugs → failed round) → fresh coder fixes → owner merges.
...
**Coder model (owner, 2026-09-24).** Coders run on Opus 5.5 by default, for every issue size and risk. Before this, coders started on Sonnet and only risky issues went to Opus; the 4 weeks to 2026-09-24 put every failed round on those risky issues (#122, #147, #148, #165, #234), and the owner prefers first-round quality to Sonnet's lower price per run. The coder's `round.sh` comment records the model.
...
- **2 attempts per tier.** **Opus 5.5** ×2 → **Fable** ×2 (label `escalated:fable`) → **owner** (label `needs-owner`). After a tier's 2nd failure the coder posts `scripts/gh/round.sh <issue> escalation 2/2 <model> "…"`, messages `team-lead` and stops; the lead starts a fresh agent with `model` set to the next tier, same branch and worktree, pointed at those comments.
...
- **Escaped defects:** a defect found after merge (owner check, later work) is filed `type:bug` + `found-after-merge` with `Escaped from #<pr>` in the body. `team-metrics.sh` counts them: the pipeline's miss rate, which the round metrics can't see.
...
- **Context × turns is the cost.** The week before 2026-09-19, one coder run made 809 model calls at a median 473k-token context (peak 966k) and processed 396M input tokens; the tester averaged 111k per turn and the lead 278k. Two hard caps hold this down: `maxTurns` in each agent's frontmatter (coders 150, tester 80, BA 100; eco-review 5 lowered them: no run went past 108 or 70 in three windows; a capped run returns partial output and the lead resumes it or starts a fresh agent from the WIP commit) and `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=25` in `.claude/settings.json` (compaction at 25% of the window, 250k; at 30% no run reached it; back to 30 if a failed round traces to a compaction; for the lead and every subagent). Agents keep context small by reading narrowly, batching independent calls in one turn, and committing WIP checkpoints so a cap loses nothing. `/eco-review` watches whether these move the numbers without raising failed rounds.
...
- **Fix rounds read less.** A fresh agent on a fix round reads the issue, the PR context and the files the findings name; not the architecture issue or the playbook again. Post `/code-review` findings as a PR comment (not only a message), each ending with `Expected:` (the behaviour the fix must produce, in one line), then start a fresh agent for the fix. #274's only failed round was a finding fixed the wrong way round (bug #275). Resume the original coder only if it is still `running` when the review lands.
...
- **Model tiers.** Opus 5.5 for coders, the lead and the BA; Sonnet 5 for the tester; Fable only for escalations, design decisions and an epic's BA design. Switch the lead with `/model` per task, not per session.
- **Test cadence (owner, 2026-09-24).** Coders write the tests but don't run them: they run only lint and the type check or build (web: `npm run lint` plus `npm run build` or `tsc --noEmit`; API: `dotnet build -warnaserror`), push and hand off without waiting on CI. Test output is large and a coder on a big context pays for every line of it. After a coder's "done" the lead runs `wait-ci.sh`; if CI is red, `scripts/gh/ci-failures.sh <pr> --post` posts the failing tests (name, file:line, error) as one PR comment and a fresh coder starts from it, with no tester run (#218 spent 4 tester runs, #220 3, only reading red CI). The tester starts only on green CI. A red CI caught this way goes back as a `review-fix` round and does not count against the tier's two attempts; failed acceptance criteria and filed bugs still do.
- **Never duplicate CI.** The tester relies on CI and spends its round on the acceptance criteria and what CI cannot do (RTL, keyboard, offline, two tabs, exploratory). If CI is red when it starts, the tester runs `ci-failures.sh <pr> --post` and stops.
