...
Pipeline: owner picks an issue → coder in its own worktree (`feat/<issue>-<slug>`) → PR `Closes #n` → owner runs `/code-review` → fresh coder fixes findings → `tester` where the scope rule says so (bugs → failed round) → owner merges.
...
- **Every coder run ends with a `round.sh` comment**, not just the failures: `passed`, `failed`, `review-fix`, `polish` or `escalation`. `team-metrics.sh` counts coder runs from the transcripts and prints them next to the round comments on each merged PR, so a run that recorded nothing shows up as a gap. #212 spent 4 runs, 631 turns and 107M input tokens against 0 comments, and the metrics read it as a clean one-round issue.
...
- **Measure the premise, don't argue it.** When the design check is about layout, sizing or timing, the comment must give numbers taken from the running app in the failure case the issue describes — the before value and the value the fix produces — not an argument from spec semantics. #213 is the cost of the alternative: two design-check comments that reasoned correctly-sounding CSS mechanism, both wrong, 2 failed Sonnet rounds and an Opus escalation on an S-sized issue (385 turns, 48M tokens); the round-3 comment measured four conditions in Chromium before writing any code and passed both rounds. A measurement that contradicts the plan is the cheapest round the team can buy.
...
- **Give every spawned agent a `name`**, including `/code-review`'s forks and one-off research agents. The transcripts record no link from an agent id back to its type, so an unnamed agent's tokens land in `team-metrics.sh`'s `unnamed-agent` bucket and no review can attribute them: 2,038 turns and 175M input tokens in the 2026-09-20 window.
